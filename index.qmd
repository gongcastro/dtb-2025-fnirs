...
```{python}
# | label: fig-od-short-psd-adult
# | fig-cap: Optical density power spectral density after short channel regression.
plot = od.compute_psd().plot()
```
...
```{python}
# | label: fig-od-tddr-psd-adult
# | fig-cap: Optical density power spectral density after TDDR.
plot = od.compute_psd().plot(show=False)
```

## Adult data

### Haemodynamic response (HbO, HbR)
...
# | fig-cap: Raw light intensity.
raw = mne.io.read_raw_nirx(Path("data", "infant-data"))
...
```{python}
# | label: fig-od-psd-sci-newborn
# | fig-cap: Optical density power spectral density after removing channels with low SCI.
od.info["bads"] = [ch for ch, sci in zip(od.ch_names, sci_list) if sci < sci_threshold]
plot = od.compute_psd().plot(show=False)
```
...
plot = epochs.plot(butterfly=True, scalings=1e-6, show=False)
```
